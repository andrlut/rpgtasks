...
- **Premium — P1 conversion layer shipped** (P1.1 in cloud, migration `20260707000001`): `profile.subscription_tier` (`free`/`premium`, default free; `lock_subscription_tier` trigger blocks self-grant; granted manually via Studio). Client layer in `app/lib/premium/` (`useIsPremium`, instrument gates, `FREE_LIMITS` = 10 tasks / 5 rewards / 3 skills / 3 quests, teaser + limit-modal stores) + hosts in `app/components/premium/` + paywall route `/premium?source=…`. Free limits are **enforced server-side**: `BEFORE INSERT` triggers on task/reward/skill/quest raise `free_limit_reached` (hint `free_limit`); a `psych_session` trigger gates the 6 deep instruments (`PREMIUM_INSTRUMENT_IDS` — Avaliação stays free). **No payment yet**: `PURCHASES_ENABLED=false` (CTA "Em breve"; RevenueCat = P2) and `PREMIUM_LEARN_ENABLED=false` (Learning premium badge inert).
...
- **Emailed auth links do NOT work on Android — both flows use typed OTP codes.** Supabase answers `/auth/v1/verify` with a `303` into `rpgtasks://…`, and Chrome refuses to follow a *server-initiated* redirect into a custom scheme (no user gesture in the chain) → `ERR_UNKNOWN_URL_SCHEME`, with the token already burned. Password recovery (`app/app/forgot-password.tsx`) and signup confirmation (`app/app/login.tsx`) both collect a code and call `verifyOtp({ email, token, type })`.
  - ✅ **Templates applied 2026-07-20.** Both **Reset Password** and **Confirm signup** now emit `{{ .Token }}`. They are **writable via the Management API** (`PATCH /v1/projects/{ref}/config/auth`, keys `mailer_templates_confirmation_content` / `mailer_templates_recovery_content`) — an earlier note claiming this was dashboard-only was wrong. Send the body as a file written by an editor, **never a bash heredoc**: on Windows a heredoc mangles UTF-8 and silently stores `U+FFFD` where the accents were. When verifying, decode the GET with an explicit `encoding='utf-8'` — piping into `python` reads stdin as cp1252 and reports mojibake that isn't there.
  - ⚠️ **`rate_limit_email_sent = 2`, project-wide, per hour** — `smtp_host` is null, so everything runs on Supabase's built-in mailer, shared across signup confirmation and recovery. Auth works but does not scale; a real SMTP provider (Resend/SES/Postmark) is a prerequisite for any actual launch. This is why the UI parks the user on the code screen *before* attempting a resend.
  - **An unconfirmed account was a closed loop until #281.** GoTrue reports it as a *sign-in* failure (`email_not_confirmed`), and signing up again returns the obfuscated already-registered response, which bounces back to login. `login.tsx` now routes that error into the signup code screen and resends. Every account created through the Play Store app was stuck this way — when a build "works" in Expo Go but users cannot log in, check `auth.users` for `email_confirmed_at IS NULL AND last_sign_in_at IS NULL` first.
  - **Expo Go never receives OTA updates** — it runs the `pnpm dev` bundle live, so it always shows the newest code. That is the opposite of production and makes it useless for validating what shipped. Validate on the internal APK (`preview`) or the Play Store app (`production`).
  - Code length is **not 6**. `mailer_otp_length` is a project-wide setting, currently **8**, and it governs every email template. The client cannot read it, so `app/lib/auth/otp.ts` accepts a range instead of pinning a value — pinning 6 is exactly what broke #276.
  - `resend()` accepts `'signup' | 'email_change' | 'sms' | 'phone_change'` — **not `'recovery'`**. Recovery re-sends by calling `resetPasswordForEmail` again.
  - Traps if you touch this: `setSession()` emits `SIGNED_IN`, **never** `PASSWORD_RECOVERY`, so a fragment-borne recovery link silently skips `/reset-password`; only `verifyOtp` and PKCE `exchangeCodeForSession` emit it. And GoTrue obfuscates already-registered addresses — `signUp` returns 200 with `data.user.identities === []` and sends nothing.
